...
<template>
  <div class="min-h-screen bg-fog pb-20">
    ...
    <main class="max-w-[800px] mx-auto px-6 mt-12">
      ...
      <p class="text-subheading tracking-subheading text-graphite mb-8">나의 캠핑카 예약 내역을 확인하세요.</p>
      ...
      <div v-else class="space-y-6">
        <div v-for="(rental, index) in rentals" :key="index" class="bg-snow rounded-cards p-[28px] flex flex-col md:flex-row justify-between gap-6">
          <div>
            ...
            <div class="flex flex-col gap-1 text-body-sm text-ink bg-fog p-4 rounded-lg">
              <div><span class="font-semibold text-graphite mr-2">대여일시</span> {{ formatDate(rental.startDateTime) }}</div>
              <div><span class="font-semibold text-graphite mr-2">반납일시</span> {{ formatDate(rental.endDateTime) }}</div>
            </div>
          </div>
