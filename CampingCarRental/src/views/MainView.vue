...
<template>
  <div class="min-h-screen bg-fog pb-20">
      ...
      <div class="max-w-[1200px] mx-auto px-6 h-[52px] flex items-center justify-between">
        ...
        <div class="flex items-center gap-5">
          <button
            ...
            class="text-ink hover:text-azure transition-colors"
            title="My Page"
          >
          ...
          <button
            @click="handleLogout"
            class="text-cobalt-link text-body-sm font-medium hover:underline"
          >
            Logout
          </button>
        </div>
    ...
    <div class="h-[250vh] relative" ref="heroContainer">
      <div class="sticky top-[52px] h-[calc(100vh-52px)] w-full overflow-hidden bg-ink">
        ...
        <div class="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
          <h1
            class="text-snow text-5xl md:text-7xl font-bold tracking-heading text-center mb-6 drop-shadow-lg"
          >
            Start Your Journey
          </h1>
          <p class="text-snow/90 text-xl md:text-2xl tracking-subheading drop-shadow-md">
            Scroll to explore
          </p>
        </div>
      </div>
    ...
    <main class="max-w-[1200px] mx-auto px-6 mt-12 space-y-16">
      ...
      <section v-if="selectedRegion && !selectedCompany">
        <div class="flex items-end justify-between mb-6">
          <h2 class="text-heading-sm font-bold tracking-heading-sm text-ink">
            Companies in {{ selectedRegion }}
          </h2>
        </div>

        <div v-if="loadingCompanies" class="text-graphite py-8">Loading...</div>
        <div v-else-if="companies.length === 0" class="text-graphite py-8">
        ...
        <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          >
            <div>
              <h3 class="text-[28px] font-semibold tracking-[-0.005em] leading-[1.1] text-ink mb-2">
                {{ company.companyName }}
              </h3>
              <p class="text-[17px] text-graphite mb-8 leading-[1.47] tracking-[-0.003em]">
            ...
            <div class="flex items-end justify-between">
              ...
              <button
                ...
                class="text-[17px] text-ink flex items-center gap-1 group-hover:text-cobalt-link transition-colors"
              >
                차량 보기
      ...
      <section v-if="selectedCompany" class="animate-fade-in">
        ...
        <h2 class="text-heading font-bold tracking-heading text-ink mb-2">
          {{ selectedCompany.companyName }}'s Cars
        </h2>
        ...
        <div v-else-if="cars.length === 0" class="text-graphite py-8">
          No cars available from this company.
        </div>
        ...
        <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          >
            <div class="w-full aspect-[4/3] bg-fog flex items-center justify-center">
              <img
                v-if="car.carImageUrl"
                :src="getImageUrl(car.carImageUrl)"
                :alt="car.carName"
            ...
            <div class="p-[28px] flex flex-col flex-grow justify-between">
              <div>
                <h3 class="text-[24px] font-semibold tracking-[-0.015em] text-ink mb-1">
                  {{ car.carName }}
                </h3>
                <p class="text-[14px] text-graphite mb-4">
                  {{ car.carNumber }} • {{ car.numberOfRider }} Riders
                </p>
              ...
              <div class="mt-8 flex items-center justify-between border-t border-silver-mist pt-6">
                <div class="flex flex-col">
                  ...
                  <span class="text-[20px] font-semibold text-ink"
                    >₩{{ car.carRentalCost.toLocaleString() }}</span
                  >
                </div>
                <button
                  @click="goToRental(car)"
                  class="bg-azure text-snow px-5 py-2 rounded-buttons text-[17px] font-medium transition-transform active:scale-[0.98]"
                >
                  Rent
                </button>
              </div>
      ...
      <section class="py-16 border-t border-silver-mist">
        <h2 class="text-heading font-bold tracking-heading text-ink mb-10 text-center">
          Why Choose Us
        </h2>
        <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
          <div class="flex flex-col items-center text-center p-[28px] bg-snow rounded-cards">
            <div class="text-4xl mb-4">🚐</div>
            <h3 class="text-[20px] font-semibold tracking-[-0.01em] text-ink mb-2">
              Premium Vehicles
            </h3>
            <p class="text-[17px] text-graphite leading-[1.47]">
              Experience the outdoors with our top-tier, fully equipped camping cars.
            </p>
          </div>
          <div class="flex flex-col items-center text-center p-[28px] bg-snow rounded-cards">
            ...
            <p class="text-[17px] text-graphite leading-[1.47]">
              Pick up and drop off your vehicle at any of our branches nationwide.
            </p>
          </div>
          <div class="flex flex-col items-center text-center p-[28px] bg-snow rounded-cards">
            <div class="text-4xl mb-4">🛡️</div>
            ...
            <p class="text-[17px] text-graphite leading-[1.47]">
              Our customer service team is always here to assist you during your journey.
            </p>
          </div>
        </div>
      </section>
    ...
    <footer class="bg-ink text-snow py-12 mt-20">
      <div class="max-w-[1200px] mx-auto px-6 grid grid-cols-1 md:grid-cols-4 gap-8">
        <div class="md:col-span-2">
          ...
          <p class="text-body-sm text-snow/70 max-w-sm">
            Your ultimate partner for road trips and outdoor adventures. Start your journey with the
            best camping cars available in Korea.
          </p>
        </div>
        <div>
      ...
      <div
        class="max-w-[1200px] mx-auto px-6 mt-12 pt-8 border-t border-snow/20 text-center text-caption text-snow/50"
      >
